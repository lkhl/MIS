    ...
    def load_model(model, pretrained=None, all_match=True, network='resnet101'):
        ...
        if all_match:
            ...
            if network == 'hrnet_plus':
                ...
                for name, m in model.named_parameters():
                    if 'relative_position_bias_table' in name and 'embed' not in name:
                        ...
                        if target_size != ckpt_size:
                            ...
                            reshape_ckpt = (
                                pretrained_dict[name].permute(1, 0).reshape(
                                    1, head_num, ckpt_size, ckpt_size))
                            inter_ckpt = (
                                torch.nn.functional.interpolate(
                                    reshape_ckpt,
                                    size=(target_size, target_size),
                                    mode='bilinear',
                                ).reshape(head_num, -1).permute(1, 0))
                            scale = 1
                            inter_ckpt *= scale
                ...
                load_dict = {
                    '.'.join(k.split('.')[1:]): v
                    for k, v in pretrained_dict.items() if '.'.join(k.split('.')[1:]) in model_dict
                }
            else:
                load_dict = {
                    '.'.join(k.split('.')[1:]): v
