        ...
        for i in range(num_branches):
            branches.append(
                self._make_one_branch(
                    ...
                    num_window_sizes,
                    num_mlp_ratios,
                    drop_paths=[_ * (2**i) for _ in drop_paths] if os.environ.get(
                        'multi_res_drop_path', False) else drop_paths,
                ))
                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    ...
                                    nn.SyncBatchNorm(num_inchannels[j], momentum=BN_MOMENTUM),
                                    nn.Conv2d(
                                        num_inchannels[j],
                                    ...
                                    nn.SyncBatchNorm(num_outchannels_conv3x3, momentum=BN_MOMENTUM),
                                ))
                        else:
                ...
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            ...
                            nn.SyncBatchNorm(num_channels_cur_layer[i], momentum=BN_MOMENTUM),
                            nn.ReLU(inplace=True),
                        ))
                else:
