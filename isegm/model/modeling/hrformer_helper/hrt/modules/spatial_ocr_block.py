...
class SpatialGather_Module(nn.Module):
    ...
    def forward(self, feats, probs, gt_probs=None):
        if self.use_gt and gt_probs is not None:
            ...
            ocr_context = (torch.matmul(gt_probs, feats).permute(0, 2,
                                                                 1).unsqueeze(3))  # batch x k x c
            return ocr_context
        else:
            batch_size, c, h, w = (
                probs.size(0),
            ...
            ocr_context = (torch.matmul(probs, feats).permute(0, 2,
                                                              1).unsqueeze(3))  # batch x k x c
            return ocr_context
...
class SpatialOCR_Module(nn.Module):
    ...
    def __init__(
        ...
        self.use_oc = use_oc
        self.fetch_attention = fetch_attention
        self.object_context_block = ObjectAttentionBlock2D(in_channels, key_channels, scale, use_gt,
                                                           use_bg, fetch_attention, bn_type)
        if self.use_bg:
            if self.use_oc:
...
class _MultiheadObjectAttentionBlock(nn.Module):
    ...
    def forward(self, x, proxy):
        ...
        query = self.f_pixel(x).view(batch_size, self.num_heads,
                                     self.key_channels // self.num_heads, -1)
        query = query.permute(0, 1, 3, 2)  # (b, nH, N, C//nH)
        key = self.f_object(proxy).view(batch_size, self.num_heads,
                                        self.key_channels // self.num_heads, -1)
        value = self.f_down(proxy).view(batch_size, self.num_heads,
                                        self.key_channels // self.num_heads, -1)
