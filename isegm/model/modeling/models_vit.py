...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp = Mlp(
            in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=mlp_drop)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1,
                                                  embed_dim))  # learnable positional embedding
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                act_layer=act_layer) for _ in range(depth)
        ])
        ...
        if cls_feature_dim:
            ...
            self.pre_logits = nn.Sequential(
                OrderedDict([('fc', nn.Linear(embed_dim, cls_feature_dim)), ('act', nn.Tanh())]))
        else:
    ...
    def unpatchify(self, x):
        ...
        x = x.permute((0, 1, 3, 2, 4, 5)).contiguous().view(B // (win_h * win_w), win_h * win_w * N,
                                                            C)
