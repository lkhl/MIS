...
from isegm.utils.misc import clamp_bbox, expand_bbox, get_bbox_from_mask, get_labels_with_sizes
...
class ZoomIn(DualTransform):
    ...
    def apply_to_mask(self, mask, selected_object, bbox, **params):
        if selected_object is None:
            if self.always_resize:
                mask = F.resize(
                    mask, height=self.height, width=self.width, interpolation=cv2.INTER_NEAREST)
            return mask
        ...
        new_mask = F.resize(
            new_mask, height=self.height, width=self.width, interpolation=cv2.INTER_NEAREST)
        return new_mask
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        if is_mask_layer:
            for layer_indx in range(instances.shape[2]):
                ...
                candidates.extend([(layer_indx, obj_id) for obj_id, area in zip(labels, areas)
                                   if area > self.min_area])
        else:
            labels, areas = get_labels_with_sizes(instances)
            candidates = [obj_id for obj_id, area in zip(labels, areas) if area > self.min_area]
    ...
    def get_transform_init_args_names(self):
        return ('height', 'width', 'bbox_jitter', 'expansion_ratio', 'min_crop_size', 'min_area',
                'always_resize')
