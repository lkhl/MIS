...
class ISTrainer(object):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.is_master:
                ...
                for k, v in self.loss_cfg.items():
                    if '_loss' in k and hasattr(v, 'log_states') and self.loss_cfg.get(
                            k + '_weight', 0.0) > 0:
                ...
                self.sw.add_scalar(
                    ...
                    value=self.lr
                    if not hasattr(self, 'lr_scheduler') else self.lr_scheduler.get_lr()[-1],
                    global_step=global_step)
    ...
    def validation(self, epoch):
        ...
        tbar = tqdm(
            self.val_data, file=self.tqdm_out, ncols=100) if self.is_master else self.val_data
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            loss = 0.0
            loss = self.add_loss('instance_loss', loss, losses_logging, validation, lambda:
                                 (output['instances'], batch_data['instances']))
            loss = self.add_loss('instance_aux_loss', loss, losses_logging, validation, lambda:
                                 (output['instances_aux'], batch_data['instances']))
    ...
    def save_visualization(self, splitted_batch_data, outputs, global_step, prefix):
        ...
        def _save_image(suffix, image):
            cv2.imwrite(
                str(output_images_path / f'{image_name_prefix}_{suffix}.jpg'), image,
        ...
        image_with_points = draw_points(image_with_points, points[self.max_interactive_points:],
                                        (0, 0, 255))
