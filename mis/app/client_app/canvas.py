...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.canvas.bind('<ButtonPress-3>',
                         self.__right_mouse_button_pressed)  # remember canvas position
        self.canvas.bind('<ButtonPress-2>',
                         self.__right_mouse_button_pressed)  # remember canvas position (MacOS)
        self.canvas.bind('<ButtonRelease-3>',
                         self.__right_mouse_button_released)  # remember canvas position
        self.canvas.bind('<ButtonRelease-2>',
                         self.__right_mouse_button_released)  # remember canvas position (MacOS)
        self.canvas.bind('<B3-Motion>',
                         self.__right_mouse_button_motion)  # move canvas to the new position
    ...
    def __show_image(self):
        ...
        if int(x2 - x1) > 0 and int(y2 - y1) > 0:  # show image if it in the visible area
            ...
            crop_w, crop_h = math.ceil(sx2 - sx1 + 2 * border_width), math.ceil(sy2 - sy1 +
                                                                                2 * border_width)
