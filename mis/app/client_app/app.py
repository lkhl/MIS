'''
Adapted from SimpleClick (https://github.com/uncbiag/SimpleClick/tree/main/simpleclick/interactive_demo)
'''
...
from .wrappers import BoundedNumericalEntry, FocusButton, FocusCheckButton, FocusHorizontalScale, \
    FocusLabelFrame


class MISClientApplication(ttk.Frame):

    def __init__(self, master, args, model):
        super().__init__(master)
        self.master = master
        master.title('MIS Demo')
        master.withdraw()
    ...
    def _init_state(self):
        self.state = {
            'zoomin_params': {
                ...
                'target_size':
                tk.IntVar(value=448),  #tk.IntVar(value=min(400, self.limit_longest_size)),
    ...
    def _add_canvas(self):
        ...
        self.canvas = tk.Canvas(
            self.canvas_frame, highlightthickness=0, cursor='hand1', width=400, height=400)
    ...
    def _add_buttons(self):
        ...
        FocusCheckButton(
            ...
            text='Fixed crop',
            command=self._reset_predictor,
            variable=self.state['zoomin_params']['fixed_crop']).grid(
                row=1, column=0, padx=10)
        tk.Label(
            self.zoomin_options_frame, text='Skip clicks').grid(
                row=0, column=1, pady=1, sticky='e')
        tk.Label(
            self.zoomin_options_frame, text='Target size').grid(
                row=1, column=1, pady=1, sticky='e')
        tk.Label(
            self.zoomin_options_frame, text='Expand ratio').grid(
        ...
        BoundedNumericalEntry(
            ...
            max_value=None,
            vartype=int,
            name='zoom_in_skip_clicks').grid(
                row=0, column=2, padx=10, pady=1, sticky='w')
        BoundedNumericalEntry(
            self.zoomin_options_frame,
            ...
            vartype=int,
            name='zoom_in_target_size').grid(
                row=1, column=2, padx=10, pady=1, sticky='w')
        BoundedNumericalEntry(
            self.zoomin_options_frame,
            ...
            max_value=2.0,
            vartype=float,
            name='zoom_in_expansion_ratio').grid(
                row=2, column=2, padx=10, pady=1, sticky='w')
    ...
    def _load_mask_callback(self):
        if not self.controller.net.with_prev_mask:
            messagebox.showwarning(
                'Warning', "The current model doesn't support loading external masks. "
    ...
    def _reset_predictor(self, *args, **kwargs):
        ...
        net_clicks_limit = None if brs_mode == 'NoBRS' else self.state['predictor_params'][
            'net_clicks_limit'].get()
        ...
        predictor_params = {
            ...
            'lbfgs_params': {
                'maxfun': self.state['lbfgs_max_iters'].get()
            }
        }
