...
try:
    ...
    msg = ('import eigen_tensor_avoid_stl_array FAILED, while '
...
def test_round_trip(m):
    ...
    np.testing.assert_array_equal(tensor_ref[:, ::-1, :],
                                  m.round_trip_tensor(tensor_ref[:, ::-1, :]))
...
def test_doc_string(m, doc):
    ...
    assert (doc(
        m.copy_fixed_tensor) == 'copy_fixed_tensor() -> numpy.ndarray[numpy.float64[3, 5, 2]]')
    assert (doc(m.reference_const_tensor) ==
