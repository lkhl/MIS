        ...
        for order in ['C', 'F']:
            for shape in [(2, 2), (1, 3, 1, 1), (1, 1, 1), (0, )]:
                if not noconvert:
                    ...
                    should_raise = dtype.name != 'float64' or (contiguity is not None
...
def test_pass_array_pyobject_ptr_return_sum_str_values_ndarray():
    ...
    assert (m.pass_array_pyobject_ptr_return_sum_str_values(
        np.array(WrapWithPyValueHolder(-3, 'four', 5.0), dtype=object)) == '-3four5.0')


def test_pass_array_pyobject_ptr_return_sum_str_values_list():
    # Intentionally all temporaries, do not change.
    assert (m.pass_array_pyobject_ptr_return_sum_str_values(
        WrapWithPyValueHolder(2, 'three', -4.0)) == '2three-4.0')
...
def test_pass_array_pyobject_ptr_return_as_list():
    ...
    assert UnwrapPyValueHolder(
        m.pass_array_pyobject_ptr_return_as_list(
            np.array(WrapWithPyValueHolder(-1, 'two', 3.0), dtype=object))) == [-1, 'two', 3.0]
